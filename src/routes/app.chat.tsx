...
import { Send, Lock, ArrowLeft, Users, MessageCircle, Paperclip, Eye, EyeOff, Clock, Play } from "lucide-react";
...
function LiveChatPage() {
  ...
  const openPrivateMeetingChat = (participantId: number, title: string) => {
    ...
    setActiveChatTitle(`Private: ${title}`);
  };
  ...
  const showLockGate = userStatus !== "approved";

  if (showLockGate) {
    return (
      <Card className="flex h-[calc(100vh-10.5rem)] flex-col items-center justify-center text-center space-y-4 px-6 border border-border/50">
        ...
        <div>
          <h3 className="font-semibold text-sm">Live Chat Locked</h3>
          <p className="text-xs text-muted-foreground mt-1 max-w-[250px] leading-relaxed">
            You must be an approved participant of this meeting to access the live chat features.
          </p>
        </div>
        <Badge variant="outline" className="border-amber-500/30 text-amber-500 bg-amber-500/10 capitalize">
          Status: {userStatus}
        </Badge>
      </Card>
    );
  ...
  return (
    <Card className="flex h-[calc(100vh-10.5rem)] flex-col border border-border/50 overflow-hidden">
      {selectedConversationId ? (
        ...
        <div className="flex flex-1 flex-col h-full bg-background/50">
          <CardHeader className="border-b py-3 px-4 flex flex-row items-center gap-3 bg-muted/10">
            ...
            <div className="flex-1 min-w-0">
              ...
              <CardDescription className="text-[10px] truncate">In-Meeting Live Chat</CardDescription>
            </div>
                        ...
                        <div className={cn("rounded-lg px-3 py-2 text-sm space-y-1.5 relative overflow-hidden", mine ? "bg-primary text-primary-foreground" : "bg-muted")}>
                          {!mine && (
                            <div className="flex items-center gap-1.5 text-[10px] opacity-80 font-bold">
                              ...
                              {m.sender_telegram_id === 9999 && <Badge variant="outline" className="h-3.5 px-1 text-[9px] border-amber-500/50 text-amber-500 bg-amber-500/10">Host</Badge>}
                            </div>
                          )}
        ...
        /* Meeting Sub-Tabs Panels (Central Chat Room, Directory, Private Chats List) */
        <div className="flex flex-1 flex-col h-full">
          {/* Sub Navigation */}
          <div className="flex border-b border-border/50 bg-muted/20">
            <button
              onClick={() => { haptic?.selectionChanged(); setSubTab("central"); }}
              className={cn(
            ...
            <button
              onClick={() => { haptic?.selectionChanged(); setSubTab("directory"); }}
              className={cn(
                "flex-1 py-3 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors border-b-2",
                subTab === "directory" ? "border-primary text-primary bg-background/50" : "border-transparent text-muted-foreground hover:bg-muted/10"
              )}
            >
              <MessageCircle className="h-4 w-4" /> Members
            </button>
            <button
              onClick={() => { haptic?.selectionChanged(); setSubTab("private-1-1"); }}
              className={cn(
                "flex-1 py-3 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors border-b-2",
